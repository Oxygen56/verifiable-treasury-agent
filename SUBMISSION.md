...
- 40 repository-wide checks passed: 33 current V2 contract/state-path/planner/live-verifier checks plus 7 retained historical V1 controls. The V2-only judge bundle reproduces the 33 current checks.
...
- The hosted judge interface can also perform a fresh, browser-side Base Sepolia check of all 26 receipt statuses, both terminal states, balances, `totalEscrowed`, and solvency without connecting a wallet or exposing a transaction path.
...
These are reproducible engineering checks, not an independent security audit or formal verification. Blockscout now publishes the V2 standard-JSON source and constructor arguments, reports verified/partial matching and unchanged bytecode, but does not report full verification. The stricter project verifier separately proves local compiler output matches the deployed runtime outside documented immutable slots.
...
6. **2:40–3:00 — evidence and honest boundary.** Run the live read-only chain check, then show 40 repository-wide checks, 64 paths, coverage, Slither triage, bytecode match, and the explicit mUSD/synthetic/non-production limitations.
...
- Blockscout source: https://base-sepolia.blockscout.com/address/0x7B92aB3D8BA17cF5f28C60E5c1FC326862dD6395?tab=contract
